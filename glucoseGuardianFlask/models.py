...
class User(db.Model, UserMixin):
    ...
    preference = db.Column(db.String(12))
...
class Swipe(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    matcher_id = db.Column(db.Intger, nullable = False)
    matched_id = db.Column(db.Intger, nullable = False)
    match_date = db.Column(db.DateTime, default=datetime.utcnow)
    match = db.Column(db.Bool, nullable = False)
